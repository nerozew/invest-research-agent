...
| P06-11L ✅ | 恢复可复现运行基线并修复 Finalizer/步骤/链路断口 | DeepSeek Finalizer 使用合法 JSON 实例、第一次无效 JSON 与 Pydantic 字段错误进行唯一一次修复；空 version 仅从 schema_version 确定性补协议元数据；Finalizer 响应进入有界失败诊断包；03_documents 补 running→succeeded；Compose 透传 failure_payload 诊断配置；角色级 structured_output_mode 保持旧调用兼容；OpenTelemetry 改用 context.attach/detach；同步过期预算与错误码测试 | 完整离线 1271 passed/19 skipped；Ruff 全绿；mypy 141 源文件全绿；本地 Docker fake smoke succeeded，8 步全成功且 08/09 报告可下载；宿主机与 worker 4 个关键源码 SHA256 完全一致；无真实 SEC/Serper/LLM 调用 | 修复请求必须基于上一次失败输出；观测内容与低基数链路分层；镜像、工作区和 Git 是三个独立版本 |
| P06-11M ✅ | 修复真实报告财务指标为空：确定性指标装配 + 年度可比期间筛选 + Analysis 工具 Schema | application/analysis_metrics.py（10 项 Decimal 指标 + SEC 来源证据）+ analysis_assembler.py（DeepSeek/Qwen 统一确定性装配）+ analysis_task.py（显式工具参数/ISO 日期 Schema）+ real_tools.py（requested_forms 感知的两期事实筛选与缓存键）+ flow_wiring.py（生产接线）+ tests/test_p06_11m_deterministic_analysis_metrics.py | FinancialCalculator 两个工具 Schema 可直接生成；请求 10-K 不被后续 10-Q 挤掉年度可比期；LLM 即使不调用计算器，最终 Pack 仍由可信 SEC facts 计算固定 10 项指标；缺输入返回 NOT_COMPUTABLE + limitations，不伪造；每项 inputs_json 含 concept/期间/accession/source_id；Qwen/DeepSeek 共用同一路径；无真实外部调用 | LLM 选事实、Python 算指标；工具函数签名就是 JSON Schema；requested_forms 必须进入缓存键 |
| P06-11N ✅ | 收口指标消费与 Direct Writer 可观测性 | writer_context_builder.py（10 项指标硬保留）+ report_draft_assembler.py（逐项报告覆盖门禁）+ direct_writer_dispatch.py/flow_wiring.py（Direct Writer 真实 usage 接入统一 LLM 指标）+ llm_full_observer.py（显式父 OTel context）+ tests/test_p06_11n_writer_metrics_observability.py | AnalysisPack 中每个指标必须以反引号代码进入 Writer 上下文与最终报告；缺任一项触发 REPORT_METRICS_MISSING 并仅重试 Writer；Direct Writer 成功/失败、耗时、input/output/cached_input token 进入统一指标且 usage 缺失不伪造 0；LLM/tool callback span 即使回调时已离开阶段上下文，仍挂到创建 Observer 时捕获的父 span；完整离线 1282 passed/19 skipped，Ruff、mypy 全绿；隔离 Docker fake 1/1 succeeded 且外部调用 0 | “已计算”不等于“已消费”；旁路调用必须在真实边界单独埋点；异步回调需显式传播 context |
