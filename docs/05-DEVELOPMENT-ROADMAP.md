...
| P06-11K-2 | 诊断包持久化 + finalize + 生命周期清理 | 诊断包 5 文件（manifest.json/execution_timeline.jsonl/stage_payloads.jsonl/validation_errors.json/failure.json）+ 成功丢弃 Payload/失败落盘 + 原子写 + 过期清理 | 失败任务能保留失败前所有阶段；成功任务 failure_payload 不落 Payload；落盘失败不覆盖原始业务异常；finalize_failure 重复调用幂等；保留天数清理；不回改变任务恢复语义 | 失败落盘、原子写、retention |
| P06-11K-3 | Research/Analysis/Writer/Quality Gate 核心阶段交接接线 | flow_wiring 接入 20 个业务边界（Request/Resolver/SEC/Serper/ResearchPack/AnalysisPack/Writer Context/LLM 调用类型/Quality Gate/Revision/ArtifactPublisher 等） | Pack 上下游交接内容可追踪；Schema 错误含 field/expected/actual；fake 注入 Research/Analysis/Writer 三类失败每类都能生成可读诊断包 | 阶段边界接线、Pydantic 错误转换 |
| P06-11K-4 | LLM/CrewAI 工具/SEC/Serper/Calculator 调用信息 + trace 关联 | LLM content/tool_calls/empty 区分、工具脱敏参数、SEC/Serper 请求结果摘要；Jaeger span 只加 diagnostic.* 低基数属性 | Jaeger/Prometheus 不包含完整 Payload；每条事件保存 trace_id/span_id；API 下载防路径穿越；不存在诊断包返回明确提示不 500；前端失败详情页显示 failure_stage/error_code/最近 10 条事件 + 下载按钮 | trace 关联、安全下载、前端诊断入口 |
| P06-11K-5 | fake Docker 故障注入 + 文档收尾 | FLOW_MODE=fake 注入 Writer Schema 失败全链路验收 + docs/05、09、12 更新 | 任务 failed、前端显示 failure_stage/error_code、下载诊断包、顺序可见 Request→ResearchPack→AnalysisPack→Writer Context→Writer Response→Validation Error、敏感字段已脱敏、Jaeger trace_id 一致、无真实 DeepSeek/SEC/Serper 调用；完整相关回归全绿 | fake 故障注入、文档收尾 |
