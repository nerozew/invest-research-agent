...
- **已提交 Phase 5 任务**：P05-01 ~ P05-11 全部 ✅；P05-12 离线部分 ✅（真实录制待授权）
- **下一个待执行任务**：**P05-12A（FLOW_MODE=fake/live 与 Production Flow Wiring）**
- **已补充到路线图的子任务**：P05-03A、P05-12A（见 docs/05-DEVELOPMENT-ROADMAP.md）
- **上下文快照**：P05-12 离线部分完成后已刷新（详见第 8 节）
...
- `docs/05-DEVELOPMENT-ROADMAP.md`：P05-01 ~ P05-11 已标 ✅；P05-12 未标 ✅（部分完成，待真实录制授权）
- `docs/09-LEARNING-LOG.md`：P05-01 ~ P05-12 学习条目已添加（含检查问题，未附答案）

## 5. 下一任务 P05-12A 的准备工作

**任务**：FLOW_MODE=fake/live 配置 + Production Flow Wiring（contract tests）
**要求**：默认 fake；普通测试/CI 不产生费用；live 缺配置 fail-fast；API Key 只从环境变量读取
**授权边界**：只做 fake/mock 离线 wiring 测试，不实际调用付费模型
**关键文件**：`src/invest_research/settings.py`、`infrastructure/wiring.py`、`flows/research_flow.py`、`infrastructure/queue/flow_adapter.py`
...
P05-12 离线部分完成后工作区已清空（所有变更已 commit）。最新 commit：`693c355`。
分支 `agent/m2-deterministic-tools` 领先 origin 22 commits（未 push）。

## 8. 新窗口继续短提示词

> 【Phase 5 继续（安全交接）】请读取 `docs/11-PHASE5-HANDOFF.md` 和
> `docs/05-DEVELOPMENT-ROADMAP.md`，确认当前在 P05-12A。
> 当前分支 `agent/m2-deterministic-tools`，已完成 P05-01 ~ P05-11（✅）+ P05-12 离线部分
> （最新 commit `693c355`），工作区干净。按 P05 自动连续执行规则从 P05-12A 开始继续；
> P05-12A 增加 FLOW_MODE=fake/live，默认 fake，只做 fake/mock 离线 wiring 测试；
> P05-13 无 live 授权保持"实现完成，等待受控 live run"；P05-14/15 可离线完成。
