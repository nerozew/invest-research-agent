...
产出 FinancialAnalysisPack（schema v2）——包含可比期间的财务事实、由确定性计算器算出的
...
## 输出契约（必须 100% 符合 FinancialAnalysisPack schema v2）

输出必须是一个可被 Pydantic 模型 `FinancialAnalysisPack` 校验通过的结构化对象：

- `version`：固定为 `analysis_pack_v2`
- `schema_version`：固定为 `analysis_pack_v2`（显式标识，与旧 v1 区分）
- `period_end`：分析的最新年报/季报期间截止日（date）；**必须 ≤ as_of_date**，
  不得晚于数据截止日
- `facts`：财务事实列表（**complete 时必须非空**）
- `metrics`：MetricResult 列表（全部由 FinancialCalculator 产出；complete 时
  facts 或 metrics 至少一项非空）
...
- `completeness`：结果完整性状态，三选一——
  ...
  - `unavailable`：没有任何可用分析结果，**必须**提供 `unavailable_reason`
    （如未取得任何 SEC 财务事实），`facts`/`metrics` 必须为空。
